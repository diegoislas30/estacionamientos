...
# Pon la URL del endpoint si quieres subir; o deja None para saltarte la subida.
ENDPOINT_UPLOAD = "https://endpoints.caabsa.com/SucursalesSINUBE_API/uploadSINUBE_Formato_EQUINSA"
SUCURSAL_HEADER = "145 COMBO VALLARTA"
...
# === LISTAR XLSX/CSV/GOOGLE SHEETS QUE NO ESTÉN PROCESADOS ===
def listar_archivos_boletaje(service, folder_id):
    q = (
        f"'{folder_id}' in parents and trashed = false "
        f"and (mimeType='application/vnd.google-apps.spreadsheet' "
        f"or name contains '.xlsx' or name contains '.csv') "
...
def descargar_archivo(service, file_id, nombre_drive, mime_type):
    ...
    # Si es archivo normal (xlsx/csv/otro descargable)
...
# === SUBIR ARCHIVO AL ENDPOINT CON PARÁMETRO DE SUCURSAL (opcional) ===
...
# === (YA NO SE USA) RENOMBRAR ARCHIVO EN DRIVE ===
...
def main():
    ...
    plaza_id = buscar_carpeta_id(service, "31. 145-COMBO VALLARTA", raiz_id)
    ...
    if not archivos:
        print("❌ No hay archivos .xlsx/.csv/Sheets para procesar.")
        return
    ...
    for archivo in archivos:
        ...
        # 3) Subir (opcional). Si ENDPOINT_UPLOAD es None, saltar subida.
        ok = True
        if ENDPOINT_UPLOAD:
            ok = subir_archivo(ENDPOINT_UPLOAD, ruta_local, SUCURSAL_HEADER, content_type_hint=content_type)
        else:
            print("ℹ️ ENDPOINT_UPLOAD=None → se omite la subida y solo se moverá a RESPALDO.")

        # 4) Si subió (o si no se sube), mover a RESPALDO **sin renombrar en Drive**
        if ok:
